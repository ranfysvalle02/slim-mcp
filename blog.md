...
*A quick note on speed, since it's the first thing a systems engineer asks.* The vector search that drives the routing is not free, but it is fast: a `$vectorSearch` query against this catalog returns in **single-digit milliseconds** — comfortably under 10 ms in practice. You must first turn the user's message into an embedding; in this demo that's a **local** model (Ollama `nomic-embed-text`, 768-dim — no keys, nothing leaves the box), which adds tens of milliseconds, and you could swap in a hosted embedding API at a similar cost. But compare that to the LLM round-trip: feeding a model ~20,000 tokens of raw schema bloat adds *seconds* of context-processing and generation latency. That massive LLM delay completely dwarfs the gateway's overhead. The database query is the fast part of the pipeline standing in for the slow part.
...
Real requests are messy — part keyword, part vibe. MongoDB lets you fuse both rankings in a single pipeline with `$rankFusion`, so a query that's half "merge this pull request" (keyword) and half "the release is blocked and the author went quiet" (meaning) gets the strengths of each:
...
One query. Two retrieval strategies, each producing its own ranked list. `$rankFusion` blends those lists by position — so you don't have to hand-tune how a text-relevance score compares to a vector-similarity score, which is the usual headache with hybrid search. No second database in the loop.

**What the gateway actually ships:** the **meaning** arm, alone. One `$vectorSearch` over the embedded catalog, and the top-k come back. We keep it to a single mechanism on purpose — the ~8.8× token win is then attributable to *one* clean move (retrieve fewer tools, text untouched), with nothing to confound it.

The keyword and hybrid pipelines above aren't hypothetical, though: they run over the *same* `tool_catalog` documents. Making the gateway hybrid is a drop-in `$search`/`$rankFusion` upgrade, not a new system — which is why it's a "go further" lever, not a missing piece.
...
On the security side, the gateway draws a hard line that has nothing to do with relevance: the **safety floor**. It leans on the upstream's own MCP annotations — a tool flagged `readOnlyHint` is always safe, one flagged `destructiveHint` is always destructive — with a conservative whole-word name match (`delete`, `destroy`, `purge`, …) as a backstop only for unannotated tools. Destructive tools are hidden in *both* directions — they never appear in a listing *and* they can't be invoked by name, so a high-scoring semantic match for "clean up the repo" can never hand the model `delete_repository`. It's the only hand-written rule in the gateway, and it's a guardrail, not an authorization model.
